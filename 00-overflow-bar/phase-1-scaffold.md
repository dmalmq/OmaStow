...
A loadable `dmalmq.omabar` directory that is a verbatim copy of the packaged bar, minus `required` host properties, with a valid third-party manifest.
...
- Write `manifest.json` with `id: "dmalmq.omabar"`, `kinds: ["bar"]`, `entryPoints.bar: "Bar.qml"`. No `omarchy.clonedFrom`.
...
Runtime: skip visual behaviour. Confirm the folder can be listed after a copy into `~/.config/omarchy/plugins/dmalmq.omabar` and `omarchy-shell shell rescanPlugins`.
